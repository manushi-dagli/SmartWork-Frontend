...
## Authentication & RBAC (Smart Work backend)

This app uses the **Smart Work** backend for authentication and role-based access control.

1. **Configure the API URL**  
   Copy `.env.example` to `.env` and set `VITE_API_URL` to your Smart Work backend base URL (e.g. `http://localhost:3000`). No trailing slash.

2. **Login**  
   Unauthenticated users are redirected to `/login`. Use the same employee/super-admin credentials as in Smart Work.
